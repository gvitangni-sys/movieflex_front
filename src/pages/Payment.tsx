...
const stripePromise = loadStripe(
  import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY ||
    "pk_test_51SBc7MPbdMUVanRJrA16CGq2vHv6VtUkdfMrbCcUOSRaKumenBxFlbxBSYrmcWAf0sNqaXCCvbY9CQTM3bP43MKG00E5UXhZcA"
);
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/test-payments/subscribe`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {isLoading
          ? "Activation en cours..."
          : `S'abonner - ${plan === "basic" ? "9.99€" : "15.99€"}/mois`}
      </NetflixButton>
    </div>
  );
};
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <CardHeader className="text-center pb-2">
              <CardTitle className="text-sm text-netflix-red">
                Premium
              </CardTitle>
            </CardHeader>
            <CardContent className="text-center space-y-1">
        ...
        <Card className="bg-gradient-card border-netflix-light-gray">
          <CardHeader className="text-center">
            <CardTitle
              className={selectedPlan === "premium" ? "text-netflix-red" : ""}
            >
          ...
          <CardContent className="text-center space-y-2">
            <div className="text-3xl font-bold text-foreground">
              {selectedPlan === "basic" ? "12.50€" : "19.99€"}
            </div>
            <div className="text-sm text-muted-foreground">par mois</div>
            <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
              <Shield className="h-4 w-4" />
              {selectedPlan === "basic"
                ? "HD • 1 écran simultané"
                : "Ultra HD • 4 écrans simultanés"}
            </div>
          </CardContent>
        </Card>
        ...
        <div className="text-center text-xs text-muted-foreground">
          ...
          <p className="mt-1">
            Système de paiement réel en mode test - Aucun vrai compte ne sera
            débité
          </p>
        </div>
      </div>
